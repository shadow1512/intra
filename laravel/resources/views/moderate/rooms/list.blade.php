                ...
                @if (count($rooms))
                    @foreach($rooms as $item)
                        <div class="row">
                            ...
                            <div class="col-md-2">@if (is_null($item->available)) (<span style="color:red">требуется подтверждение</span>)@endif</div>
                            <div class="col-md-4">@if ((is_null($item->available))  &&  ($item->numbookings   >   0)) Для {{$item->numbookings}} <a href="{{route('moderate.rooms.bookinglist', ["id"   =>  $item->id])}}">требуется подтверждение</a> @endif</div>
