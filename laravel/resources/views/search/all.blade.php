...
@if (count($users)  ||  count($deps) || count($news) || count($books) || count($razdels))
    @if (count($users)  &&  !count($deps) && !count($news) && !count($books) && !count($razdels))
            <div class="search-res_cnt_i">
                <div class="h __h_m search-res_cnt_i_b_h">{{count($users)}} {{mb_strtolower(getWordInCorrectForm(count($users),  "сотрудник"))}}</div>
                @if (count($users))
            ...
            <div class="search-res_cnt_i __no-pad">
                ...
                @if (count($deps))
                    <div class="search-res_cnt_i_b">
                        <div class="h __h_m search-res_cnt_i_b_h">{{count($deps)}} {{mb_strtolower(getWordInCorrectForm(count($deps),  "отдел"))}}</div>
                        <ul class="docs_lst">
                            @foreach ($deps as $dep)
                ...
                @if (count($news))
                    <div class="search-res_cnt_i_b">
                        <div class="h __h_m search-res_cnt_i_b_h">{{count($news)}} {{mb_strtolower(getWordInCorrectForm(count($news),  "новость"))}}</div>
                        <ul class="news_ul">
                            @foreach ($news as $new)
                ...
                @if (count($razdels))
                    <div class="search-res_cnt_i_b">
                        <div class="h __h_m search-res_cnt_i_b_h">{{count($razdels)}} {{mb_strtolower(getWordInCorrectForm(count($razdels),  "раздел"))}} библиотеки</div>
                        <ul class="docs_lst">
                ...
                @if (count($books))
                    <div class="search-res_cnt_i_b">
                        <div class="h __h_m search-res_cnt_i_b_h">{{count($books)}} {{mb_strtolower(getWordInCorrectForm(count($books),  "книга"))}} из библиотеки</div>
