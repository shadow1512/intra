    ...
    <div class="modal-w">
        <div class="modal-cnt __changes">
            ...
            <div class="profile_form_submit">
                <div class="h light_h __h_s __margin-bottom_l">Ожидайте подтверждения модератором@if(!is_null($moderator)): <a href="{{route('people.unit',   ["id"   =>  $moderator->id])}}">{{$moderate->lname}} {{mb_substr($moderate->fname, 0, 1, "UTF-8")}}. @if(!empty($moderate->mname)) {{mb_substr($moderate->mname, 0, 1, "UTF-8")}}.@endif</a><br /> После подтверждения новые данные профиля станут видны остальным сотрудникам.</div>
