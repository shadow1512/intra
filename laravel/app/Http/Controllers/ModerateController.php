    ...
    public function dinnermenuupdate($filename) {
        ...
        if($numdays >   0) {
            for ($i =   0;  $i  <   $numdays;   $i++) {
                ...
                if(isset($dataArray[1]["C"]) &&   (mb_strtolower($dataArray[1]["C"], "UTF-8")   ==  "обед") && is_integer(\PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($dataArray[2]["A"])) && is_integer($dataArray[1]["D"])) {
                    $date_menu  =   date("Y-m-d", \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($dataArray[2]["A"]));
