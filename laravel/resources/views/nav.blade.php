    ...
    <div class="main_top_phones">
        <div class="main_top_phones_people">
            ...
            <ul class="main_top_phones_lst">
                ...
                <li class="main_top_phones_lst_i"><a href="{{route('people.unit', ["id" =>  $item->id])}}" class="main_top_phones_lst_i_lk" title="{{$item->lname}} {{$item->fname}} {{$item->mname}}">{{$item->lname}} {{mb_substr($item->fname,   0,  1)  .   "."}} {{mb_substr($item->mname, 0,  1)  .   "."}} ( @if($item->ip_phone) @if(Auth::check() && !is_null(Auth::user()->ip_phone)) <a href="{{route("people.call", ["id"   =>  $item->id])}}" class="dynamic_call">{{$item->ip_phone}}</a> @else {{$item->ip_phone}} @endif @if($item->phone) или {{$item->phone}} @endif @else {{$item->phone}} @endif )</a></li>
                    @endforeach
            </ul>
                @else
