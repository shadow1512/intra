...
@endphp
        <div class="reserve_h">
            ...
            <div class="reserve_time @if($kitchen_booking)__grey @endif">@if($kitchen_booking   ||  (\Carbon\Carbon::now()->format("H:i")   >   Config::get('dinner.last_time'))) Запись на сегодня завершена @else До конца записи: <strong>@if(floor($lasttime->diffInMinutes($currtime)/60)){{floor($lasttime->diffInMinutes($currtime)/60)}}</strong> {{mb_strtolower(getWordInCorrectForm(floor($lasttime->diffInMinutes($currtime)/60),  "час"))}} @endif <strong>{{$lasttime->diffInMinutes($currtime)%60}}</strong> {{mb_strtolower(getWordInCorrectForm($lasttime->diffInMinutes($currtime)%60,  "минута"))}} @endif</div>
        </div>
